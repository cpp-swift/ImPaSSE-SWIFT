...
class Competition(dict):
    ...
    def save(self, db_session):
        for team_dict in self['teams']:
            ...
            if 'services' in team_dict:
                for service_dict in team_dict['services']:
                    ...
                    for environment_dict in service_dict['environments']:
                        ...
                        if 'invert' in environment_dict:
                            environment_obj.invert = environment_dict['invert']
            ...
            if 'scs' in team_dict:
                for scs_dict in team_dict['scs']:
                    logger.info("\tCreating {0} SCS".format(scs_dict['configuration_text']))
                    scs_obj = SCS(
                        team=team_obj,
                        points=scs_dict['points'],
                        check_name=scs_dict['check_name'],
                        host=scs_dict['host'],
                        hostname=scs_dict['hostname'],
                        port=scs_dict['port'],
                        configuration_text=scs_dict['configuration_text']
                    )
                    if 'correct' in scs_dict:
                        scs_obj.correct = scs_dict['correct']
                    if 'accounts' in scs_dict:
                        for account_dict in scs_dict['accounts']:
                            db_session.add(Account(username=account_dict['username'], password=account_dict['password'], scs=scs_obj))
                    for environment_dict in scs_dict['environments']:
                        environment_obj = Environment(scs=scs_obj, matching_content=environment_dict['matching_content'])
                        if 'invert' in environment_dict:
                            environment_obj.invert = environment_dict['invert']
                        db_session.add(environment_obj)
                        if 'properties' in environment_dict:
                            for property_dict in environment_dict['properties']:
                                db_session.add(Property(environment=environment_obj, name=property_dict['name'], value=property_dict['value']))
                    if 'worker_queue' in scs_dict:
                        scs_obj.worker_queue = scs_dict['worker_queue']
                    db_session.add(scs_obj)
            db_session.commit()
