...
class Engine(object):
    ...
    def run(self):
        ...
        while not self.is_last_round():
            ...
            scs_list = self.session.query(SCS).all()[:]
            ...
            for scs in scs_list:
                check_class = self.check_name_to_obj(scs.check_name)
                if check_class is None:
                    raise LookupError(
                        "Unable to map service to check code for "
                        + str(scs.check_name)
                ...
                logger.debug(
                    ...
                    + scs.team.name
                    + " - "
                    + scs.configuration_text
                    + " check to queue"
                )
                environment = random.choice(scs.environments)
                check_obj = check_class(environment)
                command_str = check_obj.command()
                job = Job(environment_id=environment.id, command=command_str)
                task = execute_command.apply_async(
                    args=[job], queue=scs.worker_queue
                )
                team_name = environment.scs.team.name
                if team_name not in task_ids:
                    task_ids[team_name] = []
                task_ids[team_name].append(task.id)
                ...
                for team_name, task_ids in task_ids.items():
                    for task_id in task_ids:
                        ...
                        if task.result["errored_out"]:
                            ...
                            if environment.invert: # if invert, then fail when content is detected
                                if re.search(
                                    ...
                                    if environment.scs_id and not re.search('^SUCCESS', task.result["output"]):
                                        result = False
                                        reason = CHECK_FAILURE_TEXT
                            else:
                                if re.search(
                        ...
                        elif environment.scs:
                            if environment.scs.team.name not in teams:
                                teams[environment.scs.team.name] = {
                                    "Success": [],
                                    "Failed": [],
                                }
                            if result:
                                teams[environment.scs.team.name]["Success"].append(
                                        'SCS id: ' + str(environment.scs.id)
                                )
                            else:
                                teams[environment.scs.team.name]["Failed"].append(
                                        'SCS id: ' + str(environment.scs.id)
                                )

                            check = Check(scs=environment.scs, round=round_obj)
                        # Grab the first 35,000 characters of output so it'll fit into our TEXT column,
                        # which maxes at 2^32 (65536) characters
                        check.finished(
                            result=result,
                            reason=reason,
                            output=task.result["output"][:45000],
