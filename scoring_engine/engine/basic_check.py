...
class BasicCheck(object):
    def __init__(self, environment):
        ...
        self.set_properties()
        if self.environment.service:
            self.host = self.environment.service.host
            self.port = self.environment.service.port
        elif self.environment.scs:
            self.host = self.environment.scs.host
            self.port = self.environment.scs.port
    ...
    def get_random_account(self):
        if self.environment.service:
            ...
            return random.choice(self.environment.scs.accounts)
