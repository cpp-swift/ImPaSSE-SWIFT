...
def update_scs_data(team_id=None):
    from scoring_engine.web.views.api.team import api_scs
    if team_id is not None:
        cache.delete_memoized(api_scs, str(team_id))
    else:
        cache.delete_memoized(api_scs)

def update_scs_data_checks(scs_id=None):
    from scoring_engine.web.views.api.scs import scs_get_checks
    if scs_id is not None:
        cache.delete_memoized(scs_get_checks, str(scs_id))
    else:
        cache.delete_memoized(scs_get_checks)
