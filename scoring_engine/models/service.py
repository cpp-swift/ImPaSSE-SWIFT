...
from sqlalchemy import Column, Integer, String, ForeignKey, desc, func, case
...
class Service(Base):
    ...
    def rank(self):
        scores = (
            session.query(
                ...
                func.coalesce(func.sum(case(
                    (Check.result == 1, Service.points), else_=0)
                    )
                ,0)
                .label("score"),
            )
        ...
        if scores:
            ...
            except (ValueError,IndexError): # with one team, ranks don't populate
                return 1
        return 1
